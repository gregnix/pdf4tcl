...
package require **pdf4tcl ?0****.9****.4****.61?**
...
**::pdf4tcl::catPdf ?option value...? infile ?infile ...? outfile**
: This call concatenates PDF files into one. Currently the implementation limits the PDFs a lot since not all details are taken care of yet. Straightforward ones like those created with pdf4tcl or ps2pdf should work mostly ok. Merging keeps the catalog of the FIRST input, and with it its document information -- two files joined otherwise carry the title of part one. A merger cannot know what several documents are called together, so it asks:

- An empty value REMOVES the entry -- better no title than the wrong one. Entries not named keep whatever the first document had.
...
**-file path**
: Path to the invoice XML, read from disk. Excludes **-contents**.

**-filename name**
: The name the XML carries inside the PDF, used for the embedded file and for **DocumentFileName** in the XMP (default: **factur-x****.xml**).

**-conformance level**
: The invoice profile: **MINIMUM**, **BASIC WL**, **BASIC**, **EN 16931**, **EXTENDED** or **XRECHNUNG** (default: **EN 16931**).

**-documenttype type**
: **DocumentType** in the XMP (default: **INVOICE**).

**-version v**
: The Factur-X version (default: **1****.0**).

**-afrelationship rel**
: The **/AFRelationship** of the embedded XML (default: **Alternative**). MINIMUM and BASIC WL are not a complete invoice on their own and take **Data**.

**-namespace uri**
: The XMP namespace (default: the Factur-X 1.0 / ZUGFeRD 2.1 URI).

**-prefix p**
: The XMP prefix (default: **fx**).

**objectName facturx ?option value...?**
: Attaches an electronic invoice as Factur-X / ZUGFeRD and writes the XMP metadata the standard requires. This does in one call what **addEmbeddedFile** plus **metadata** would do by hand, and it adds the **fx:** XMP block a validating reader looks for. A valid electronic invoice is PDF/A-3 (**-pdfa** set to **3b** or **3a**); the method does not enforce that, but a file written without it will not pass an invoice validator. PDF/A-1 forbids embedded files outright, so **addEmbeddedFile** refuses there. Either **-contents** or **-file** has to be given, not both.

```tcl
# Every font has to be embedded -- PDF/A refuses the standard 14, so a
# document set in Helvetica fails validation whatever else is right.
pdf4tcl::loadBaseTrueTypeFont BaseSans "FreeSans.ttf"
pdf4tcl::createFontSpecCID BaseSans Uni
set pdfobject [::pdf4tcl::new %AUTO% -paper a4 -pdfa 3b]
$pdfobject startPage
$pdfobject setFont 12 Uni
...
$pdfobject facturx -file "factur-x.xml" -conformance "EN 16931"
$pdfobject write -file invoice.pdf
...
**objectName drawTextBox x y width height str ?option value...?**
: Draw the text string *str* wrapping at blanks and tabs so that it fits within the box defined by *x*, *y*, *width* and *height*. An embedded newline in *str* causes a new line in the output. If *str* is too long to fit in the specified box, it is truncated and the unused remainder is returned. A line may also break after a hyphen that joins two words, so **Donau-dampfschiff** keeps the hyphen at the end of the line. Since 0.9.4.60 a hyphen preceded by anything but a letter or digit does not break: **Der Betrag -5 Euro** used to leave the sign behind on one line and put the number on the next. Since 0.9.4.61 a soft hyphen (**­**, U+00AD) marks a place where the line MAY break. It is a suggestion, not a character: it is printed only where the break actually happens, and then as a hyphen. Everywhere else it leaves no trace and adds nothing to the width, so a text full of suggestions wraps exactly like the same text without them. Up to 0.9.4.60 it was printed wherever it stood and did not break, so **Silben­trennung** came out as one word with a stray hyphen in the middle. The hyphen added at a break is counted against the box, so the line does not stick out by its width.
...
- Since 0.9.4.60 an image placed below 150 dpi adds a note to **::pdf4tcl::warnings**: the pixel count and the target size are both known here, so the resolution follows. Nothing is refused -- a screenshot at 96 dpi may be exactly what was wanted. Set **::pdf4tcl::imageDpiWarn** to another threshold, or to 0 to turn it off. A form XObject placed with **putImage** is never reported, since its dimensions are points rather than pixels.
**-compress boolean**
