...
Since 0.9.4.49 the same holds with `setLigatures 1`: text is shaped first
and kerned afterwards, and `getStringWidth` measures that same run.

```
Unicode -> glyphs -> ligatures -> kerning -> TJ
```

Before, the string was split on the glyphs *before* substitution, so `ffi`
in Carlito was drawn as `f` + kern + `fi` rather than the `ffi` glyph, and
the width was summed per character while a ligature was drawn. See
[`howto-ligatures.md`](howto-ligatures.md).
...
Since 0.9.4.49 the flag is read **per lookup**, not per face. A lookup
that does not set it kerns its pairs across marks as written, even when
another lookup in the same font does set it -- and pairs from the older
`kern` table never skip marks, since that table has no lookup flags.

The immediate neighbour is tried first: if the base glyph and the mark
have a pair of their own, that one wins and nothing is stepped over.
