...
proc ::pdfcheck::native::checkAttachmentNames {file} {
    set data [slurp $file]
    # Go through the trailer and the catalog, not through the whole file.
    # "/EmbeddedFiles" is not unique: demo-embedfile.pdf prints PDF source
    # onto the page as an illustration, so the string appears twice -- once
    # as drawn text, once as the real entry. Searching the file found the
    # text and reported a name tree that has no /Names array.
    set r [refOf $data Root]
    if {$r eq ""} {
        return [result SKIP "attachment reachable by name" \
                "no /Root in the trailer"]
    }
    set n [refOf [objectBody $data $r] EmbeddedFiles]
    if {$n eq ""} {
        # /EmbeddedFiles lives under /Names in the catalog.
        set namesRef [refOf [objectBody $data $r] Names]
        if {$namesRef ne ""} {
            set n [refOf [objectBody $data $namesRef] EmbeddedFiles]
        }
    }
    if {$n eq ""} {
        return [result SKIP "attachment reachable by name" \
...
proc ::pdfcheck::native::checkFonts {file} {
    ...
    while {[set i [string first "/Type /Font" $data $idx]] >= 0} {
        ...
        if {[regexp {/Subtype\s*/Type0} $obj]} {
            ...
            # A subset name carries a tag: exactly six uppercase letters
            # and a plus sign (ISO 32000-1 clause 9.6.4). Anything else in
            # front of a plus is wrong, and a reader uses the tag to tell
            # two subsets of one face apart.
            if {[string first + $name] >= 0} {
                if {![regexp {^[A-Z]{6}\+.} $name]} {
                    lappend befund "$name: subset tag is not six uppercase\
                            letters followed by +"
                }
            }
            if {[regexp {/DescendantFonts\s*\[\s*(\d+)} $obj -> dn]} {
                ...
                if {[regexp {/FontDescriptor\s+(\d+)} $nach -> fd]} {
                    ...
                    # The descriptor's FontName has to carry the same name
                    # as BaseFont, tag and all -- clause 9.6.4 names both.
                    # Writing the tag in one place and not the other is
                    # exactly the kind of half-done change that no
                    # validator catches.
                    if {[regexp {/FontName\s*/([^\s/>\]]+)} $desc -> fn]} {
                        if {$fn ne $name} {
                            lappend befund "$name: descriptor says /FontName\
                                    /$fn -- the two must match"
                        }
                    }
                } else {
